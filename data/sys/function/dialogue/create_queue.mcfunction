...
$data modify storage k-e:dialogue_queue $(node).current_value set from storage k-e:dialogue_queue $(node).list[0]

$function sys:flush/log {sys:dialogue_queue, node:"$(node)"}
