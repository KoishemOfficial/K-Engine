...
scoreboard players add K-Engine.gc.node_list.amount K-Engine.gc 1
...
$execute unless score K-Engine.gc.node_list.size K-Engine.gc matches ..1 run function sys:gc/private/_run with storage k-e:gc $(sys).node_list

$execute if score K-Engine.gc.node_list.size K-Engine.gc matches ..1 run function sys:gc/private/_clear_temp {sys:$(sys), amount:$(amount)}